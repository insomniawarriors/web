# Insomnia Warriors — Website Content

> **How to use this file:** This is the single source of truth for website text. Edit the words below; you do not need to know HTML or CSS. Everything is organized by page, then by section. After editing, run `python3 scripts/generate_site.py` to update the website files.

---

## Header (appears on every page)

- Home [Link: index.html]
- The Program [Link: program.html]
- Why CBT-I [Link: why-cbti.html]
- Meet Tierza [Link: about.html]
- Testimonials [Link: testimonials.html]
- FAQs [Link: faqs.html]
- Get Started [Link: contact.html] (button)

---

## Contact Info (used across all pages)

- Email: insomniawarriors@pm.me [Link: mailto:insomniawarriors@pm.me]
- Phone: (253) 223-1751 [Link: tel:+12532231751]
- Payment: HSA/FSA accepted · Superbills provided
- Footer tagline: Clinically grounded CBT-I treatment for chronic insomnia. Reclaim restful sleep and a higher quality of life.
- Copyright: 2026 Insomnia Warriors. All rights reserved.
- Footer credential: Tierza Clerc, OTR/L, CBT-I
- Disclaimer: This program is not a substitute for emergency medical care. If you are experiencing a medical emergency, please call 911.

---

## Page: Home

### Hero

- Headline: Insomnia is a treatable condition
- Subtitle: Without drugs, just behavioral therapy
- Description: Cognitive Behavioral Therapy for Insomnia (CBT-I) is a program for individuals experiencing chronic insomnia and co-occurring conditions such as anxiety, PTSD, depression, chronic pain, and other medical contributors. This program provides a clinically guided pathway to restore natural sleep.
- Button 1: Schedule a Consultation [Link: contact.html]
- Button 2: Learn About the Program [Link: program.html]

### Meet Tierza (preview)

- Credential tag: OTR/L · CBT-I Certified
- Credential logos: UW Medicine [Link: https://www.uwmedicine.org], Colorado State University [Link: https://www.colostate.edu]
- Heading: Meet Tierza Clerc
- Subtitle: A therapist who has walked the same path to better sleep
- Bio: With 26 years of clinical experience, Tierza combines evidence-based CBT-I with a compassionate, human-centered approach. She has personally experienced the program's life-changing results — and is dedicated to helping others do the same.
- Button: Learn More About Tierza [Link: about.html]

### Stat Bar

- Stat 1: 95% — Success Rate with Adherence
- Stat 2: 7–11 — Typical Session Program

### Who This Program Is For

- Section label: Is This You?
- Heading: Who This Program Is For
- Description: You may be a candidate for CBT-I treatment if any of these describe your experience.
- Symptom 1: You lie awake for hours despite being tired or exhausted
- Symptom 2: You wake frequently during the night and cannot fall back asleep
- Symptom 3: You wake for the day much earlier than you want
- Symptom 4: Your mind becomes alert or anxious at bedtime
- Symptom 5: You feel foggy, depleted, or that you don't function well in the daytime
- Symptom 6: You've tried sleep hygiene, supplements, or apps without lasting improvement
- Footnote: You want a structured, clinically validated method — not another short-term fix.

### Why This Works

- Section label: What Sets Us Apart
- Heading: Why This Works
- Description: A proven, evidence-based program for chronic insomnia
- Card 1 title: Evidence-Based Techniques
- Card 1 text: Clinically validated CBT-I protocols with measurable outcomes and structured treatment plans.
- Card 2 title: Trauma-Informed Care
- Card 2 text: Medically integrated treatment adapted for anxiety, PTSD, depression, chronic pain, and more.
- Card 3 title: Measurable Progress
- Card 3 text: Daily sleep diaries and structured protocols that track your improvement with real data.
- Card 4 title: Provider Collaboration
- Card 4 text: Treatment aligned and coordinated with your existing healthcare providers.
- Card 5 title: No Judgment, No Shame
- Card 5 text: Just clinically grounded care with a human-centered, compassionate approach.

### Call to Action

- Heading: Take the First Step Toward Better Sleep
- Text: A short conversation is all it takes to find out if CBT-I is right for you.
- Button: Get Started [Link: contact.html]

---

## Page: The Program

### Page Header

- Section label: Clinical CBT-I Treatment
- Heading: The Program
- Subtitle: A clinically guided pathway to restoring natural, restorative sleep

### Program Components

- Section label: What We Provide
- Heading: Program Components
- Description: A comprehensive, multi-component treatment program — not coaching, not tips, not suggestions.
- Component 1 title: Clinical Insomnia Assessment
- Component 1 text: A structured evaluation using validated tools (ISI, sleep diaries, clinical interview and more) to determine insomnia subtype, severity, and contributing factors.
- Component 2 title: CBT-I Treatment Protocol
- Component 2 text: A stepwise, evidence-based intervention including stimulus control, sleep scheduling, cognitive reframing (building healthier sleep thinking patterns and shifting thoughts that keep you awake), and behavioral modification.
- Component 3 title: Co-Occurring Condition Integration
- Component 3 text: Treatment is adapted for individuals with anxiety, PTSD, depression, chronic pain, hormonal changes, long COVID, and other medical contributors. Collaboration with your therapist, psychiatrist, or physician is integrated for most individuals.
- Footnote: This is not coaching. It is a clinical protocol requiring commitment, adherence, and ongoing evaluation. Most individuals participate in a minimum of seven sessions. Many require longer programs depending on the complexity of their insomnia, how their body responds to the therapy, and related co-occurring conditions.

### How The Program Works

- Section label: Step by Step
- Heading: How The Program Works
- Step 1 title: Complete the Clinical Assessment
- Step 1 text: We evaluate your sleep history, symptoms, medical factors, and co-occurring conditions using validated clinical tools.
- Step 2 title: Begin Your Sleep Diaries
- Step 2 text: Once determined eligible, you complete 14 days of sleep diaries before your next session. Daily sleep diaries are mandatory — they are part of what makes this an evidence-based program. You'll track your sleep changes throughout the entire program.
- Step 3 title: Receive a Clinical Treatment Plan
- Step 3 text: Your provider identifies the mechanisms driving your insomnia and outlines a structured CBT-I-based protocol tailored to your unique situation.
- Step 4 title: Engage in Weekly Treatment Sessions
- Step 4 text: Sessions include progress review, data-driven adjustments, and integration of strategies for anxiety, trauma, or medical contributors. The average session commitment is 7–11 sessions.
- Step 5 title: Achieve Sustainable, Natural Sleep
- Step 5 text: With adherence, most individuals experience measurable improvements in sleep efficiency, nighttime awakenings, and confidence in their ability to sleep.

### Note About Underlying Conditions

- Heading: A Note About Underlying Conditions
- Paragraph 1: If certain conditions are identified during the initial intake paperwork, we will recommend a PCP consult first to resolve or begin treating underlying conditions.
- Paragraph 2: Certain conditions like unaddressed sleep apnea may impact the team's ability to effectively treat chronic insomnia. Sleep apnea affects someone's ability to receive enough oxygen when they sleep, and lasting improvement doesn't happen when someone doesn't get enough oxygen while sleeping.
- Paragraph 3: We explore all underlying conditions so that your treatment is whole-person focused and comprehensive — addressing all reasons for your insomnia and your poor night's sleep.

### Call to Action

- Heading: See Yourself in This Program?
- Text: Let's talk about your sleep history and whether these steps are the right fit.
- Button: Book a Free Consultation [Link: contact.html]

---

## Page: Why CBT-I

### Page Header

- Section label: The Gold Standard
- Heading: Why CBT-I
- Subtitle: Industry gold standard for treating chronic insomnia — without medication

### CBT-I Treats the Root Cause

- Section label: Evidence-Based
- Heading: CBT-I Treats the Root Cause
- Description: CBT-I focuses on the underlying thoughts and behaviors that keep insomnia going — rather than masking the problem with medication. It helps retrain the body's natural sleep system.

- Card 1 title: Proven to Improve Sleep
- Card 1 text: Research shows CBT-I consistently improves: time to fall asleep, nighttime awakenings, overall sleep quality, daytime functioning.

- Card 2 title: No Side Effects
- Card 2 text: CBT-I achieves results superior to sleep medications, with: no dependency, no tolerance buildup, no cognitive or next-day side effects.

- Card 3 title: Long-Lasting Results
- Card 3 text: Unlike medications, CBT-I teaches skills that continue to work after treatment ends. Many patients maintain or even improve their sleep over time.

- Card 4 title: Structured & Practical
- Card 4 text: CBT-I is typically delivered over 7–10 sessions and includes practical tools such as: sleep scheduling, stimulus control (retraining bed–sleep association), cognitive restructuring (reducing sleep anxiety), relaxation techniques.

- Card 5 title: Improves Overall Well-Being
- Card 5 text: Beyond sleep, CBT-I can also improve: mood (anxiety and depression), energy and focus, quality of life.

### CBT-I vs. Medication Comparison

| Factor | CBT-I | Sleep Medication |
|---|---|---|
| Treats root cause | Yes | No |
| Long-term results | Yes | No |
| No dependency risk | Yes | No |
| No side effects | Yes | No |
| Builds lasting skills | Yes | No |

### Call to Action

- Heading: Ready to Try the Gold Standard?
- Text: You've seen the evidence. Let's discuss how CBT-I applies to your situation.
- Button: Schedule a Consultation [Link: contact.html]

---

## Page: Meet Tierza

### Page Header

- Section label: Your Therapist
- Heading: Meet Tierza Clerc
- Subtitle: A therapist who has walked the same path to better sleep

### Bio

- Credential tag: OTR/L · CBT-I Certified
- Credential logos: UW Medicine [Link: https://www.uwmedicine.org], Colorado State University [Link: https://www.colostate.edu]
- Heading: 26 Years of Clinical Experience
- Paragraph 1: Tierza Clerc has been in the medical field for 26 years — initially in medical massage for 23 years, helping people after surgeries, injuries, or trauma, and now 3 years in occupational therapy.
- Paragraph 2: Upon taking the CBT-I program as a practitioner, Tierza experienced an evidence-based, occupational therapy-driven intervention for chronic insomnia which led to managing her own insomnia towards significantly better sleep.
- Paragraph 3: The program uses Cognitive Behavioral Therapy for Insomnia (CBT-I) to improve sleep quality and daily functioning. Occupational therapy as a profession is perfectly positioned to help in the occupation of sleep.
- Paragraph 4: She is dedicated to sharing this program so others can reclaim restful sleep and a higher quality of life.

### Why Tierza Does This Work

- Section label: Approach
- Heading: Why Tierza Does This Work
- Paragraph 1: Tierza knows firsthand how insomnia can affect every part of your life — your energy, your mood, your relationships, and your ability to function during the day. She struggled with it herself before discovering CBT-I.
- Paragraph 2: That personal experience, combined with decades of clinical work, gives her a unique perspective. She doesn't just understand the science behind CBT-I — she understands what it feels like to lie awake at night wondering if anything will ever work.
- Paragraph 3: Her approach is structured and evidence-based, but always delivered with compassion, patience, and without judgment. Every person's insomnia is different, and Tierza tailors treatment to your specific needs and circumstances.

### Call to Action

- Heading: Ready to Work With Tierza?
- Text: She's helped people just like you reclaim their sleep. Start with a conversation.
- Button: Reach Out to Tierza [Link: contact.html]

---

## Page: Testimonials

### Page Header

- Section label: Client Outcomes
- Heading: Testimonials
- Subtitle: Real cases. Measurable improvement. Restored sleep.

### Testimonial 1

- Author: Andris
- Image: images/testimonial-andris.jpg
- Rating: 5 stars
- Quote: "Tierza Clerc did a really good job teaching me how to deal with my insomnia. The therapy she provided worked. I am sleeping so much better now. She has an exceptionally good understanding of insomnia because she has experienced it herself."

### Testimonial 2

- Author: J. M.
- Image: images/testimonial-jm.png
- Rating: 5 stars
- Quote: "Tierza has been a life changer! She has been a thoughtful CBT-I therapist, walking me through the steps for collecting critical data that exposed needed improvements in my sleep habits and routines. Through her careful therapy, I was able to revise these habits and routines and build consistent sleep routines. The result is Tierza helped me break the repeating cycle of insomnia that I suffered for years! I'm consistently better rested with more energy and stamina to live and enjoy life."

### Testimonial 3

- Author: John W.
- Image: images/testimonial-johnw.png
- Rating: 5 stars
- Quote: "Tierza's therapy helped me solve my lifelong insomnia. Her approach was effective and her coaching was thoughtful and tailored to my specific needs. I recommend her very highly!"

### Social Proof

- Text: 95% success rate — among individuals who adhere to and complete the program

### Privacy Note

- Text: Client photos are stock images and last names have been removed to protect patient privacy.

### Call to Action

- Heading: Your Story Could Be Next
- Text: Every client started where you are now. One conversation changed everything.
- Button: Start Your Story [Link: contact.html]

---

## Page: FAQs

### Page Header

- Section label: Common Questions
- Heading: FAQs
- Subtitle: Your questions, answered

### Question 1: What does a typical session look like?

Each session begins with a review of your sleep diary. This allows us to calculate your sleep efficiency. The remainder of the session focuses on determining if your sleep schedule needs to change, and addressing any other co-occurring conditions that need to be improved for sleep to thrive.

### Question 2: How long does treatment take?

Most individuals require a minimum of seven sessions, with many needing up to 11 sessions, depending on medical or psychological contributors.

### Question 3: What makes you different?

We combine a thoughtful, human-centered approach with clear communication and reliable results. It's not just what we do — it's how we do it that sets us apart.

### Question 4: Is this therapy?

Yes, this is evidence-based Cognitive Behavioral Therapy for Insomnia. This is not sleep coaching or a list of suggestions to maybe improve your sleep. It works for 95% of individuals who adhere to the program and complete it.

### Question 5: Do you treat co-occurring conditions?

Yes. Insomnia Warriors specializes in cases involving anxiety, PTSD, depression, chronic pain, and other medical contributors.

### Question 6: Do you accept HSA/FSA or provide superbills?

Yes. Sessions may be eligible for reimbursement depending on your plan. We provide documentation upon request. We do not take medical insurance or bill medical insurance companies as part of this program.

### Question 7: Why is a sleep diary critical?

Every session starts with a review of your sleep diary. This provides us evidence of whether we have addressed all the components of your insomnia.

When we are missing a component of your insomnia, we will plateau and your sleep improvement will stagnate. This will show in your sleep diary, and is why daily sleep diaries are so integral to this program. Every person's CBT-I therapy will look different based on their unique makeup.

### Question 8: Do you work with people who take sleep medication?

A close coordination with medical providers will be critical if you are taking sleep medications to determine if the sleep medication is for sleep only or for treating an underlying medical condition. If the medications are for sleeping only, we will work with your therapist, psychiatrist, or physician to wean you off your sleep medications so that your sleep is achieved through natural body processes. You will learn how to do this through this program.

### Question 9: What if I have tried everything?

We will discuss all the things you have tried in the past and determine which pieces were missing, or which things needed to be tried in parallel that were potentially missed.

### Call to Action

- Heading: Still Have Questions?
- Text: We're happy to answer anything — no commitment, no pressure.
- Button: Ask Us Anything [Link: contact.html]

---

## Page: Contact

### Page Header

- Section label: Get Started
- Heading: Let's Work Together
- Subtitle: Take the first step toward managing your insomnia and improving your sleep

### Reach Out

- Heading: Reach Out
- Description: Whether you're ready to begin or just want to learn more, we're here to help. No obligation, no pressure — just a conversation about your sleep.

### What to Expect

After reaching out, we'll schedule an initial consultation to discuss your sleep history and determine if CBT-I is the right fit for you. There's no commitment required to have this conversation.

### Contact Form

- Form heading: Send a Message
- Fields: First Name, Last Name, Email, Phone
- Primary Concern options: Difficulty falling asleep, Frequent nighttime waking, Waking too early, Anxiety at bedtime, Daytime fatigue / fog, Sleep medication dependence, Other
- Message prompt: How long have you been experiencing sleep difficulties? What have you tried?
- Submit button: Send Message

### After Submission

- Heading: Almost there!
- Text: Your email client should now be open with your message ready to send. Please press Send in your email app to complete your inquiry.
- Fallback: Didn't see your email app open? Click here to email us directly. [Link: mailto:insomniawarriors@pm.me]
